...
{    
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		{  	
			...
			{
				pIOCPModel->_ShowMessage("完成端口收到信息，信息类型为:%d.\n", pIoContext->m_OpType);
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	this->_ShowMessage( _T("客户端 %s:%d 连入.信息：%s."),inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port),pIoContext->m_wsaBuf.buf );
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	this->_ShowMessage(_T("投递信息:%s"), p_wbuf);
	return true;
}
